...
exports.options = {
  ...
  cwd: {
    ...
    setter: function(cwd) {
      ...
      cortex_json.package_root(this.get('cwd'), function (real_cwd) {
        // Save the origin cwd
        this.set('_cwd', cwd);
        done(null, real_cwd);
      }.bind(this));
...
function save_check_package(save) {
  ...
  if (save) {
    ...
    if (cwd === null) {
      return done('cortex.json not found, could not save dependencies');
    }
...
exports._deal_packages = function(packages, self, callback) {
  ...
  // `cortex install -g xxx`
  // No checking.
  if (g) {
    return callback(null, parsed_packages);
  }

  var save = this.get('_save');
  var real_cwd = this.get('_cwd');
  if (save) {

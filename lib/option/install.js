...
exports._deal_packages = function(packages, self, callback) {
  ...
  var g = this.get('global');
  var package_root = this.get('cwd');
  var cwd = this.get('_cwd');
  var save = this.get('_save');
  ...
  if (parsed_packages.length) {
    // NC(G), see ./README.md
    if (g) {
      return calllback(null, parsed_packages);
    }
...
exports._get_dependencies = function (cwd, self, callback) {
  cortex_json.read(cwd, function (err, pkg) {
    ...
    if (self.get('_update')) {
      return exports._deps_from_pkg(pkg, keys, callback);
    }
    ...
    fs.exists(shrinkwrap_json, function (exists) {
      ...
      exports._deps_from_pkg(pkg, keys, callback);
    });
  });
};


exports._deps_from_pkg = function (pkg, keys, callback) {
  // Including all versions
  var deps = exports._get_all_deps(pkg, keys);
  callback(null, deps);
};


exports._get_all_deps = function(pkg, keys) {
  var deps_obj = {};
...
exports._check_packages = function(packages, pkg, callback, allow_duplicate_versions) {
  if (allow_duplicate_versions) {
    ...
    // prohibited: a a 
    // allow: a@0.0.1 b
    exports._check_duplicate_versions(packages, callback, pkg.name);
  }
};
...
exports._check_duplicate_versions = function(packages, callback, pkg_name) {
  var map = {};

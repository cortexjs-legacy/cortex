...
function validate_format(path) {
    ...
    return name && semver.valid(version);
}
...
var REGEX_REPLACE_MIN = /\.min/;

module.exports = function install_and_response(server_root) {

    return function (req, res, next) {
        var path = req.path.replace(REGEX_REPLACE_MIN, '');
        ...
        if (mod_folder_exists(server_root, path) && !fs.exists(file) ) {

...
publish.prepare = function(options, callback) {
    ...
    if(options.tar){
        options.cwd = temp_package;

        async.series([
            function (done) {
                self.extract_tar(options, done);
            },

            function (done) {
                self.check_package_file(options, done);
            }
...
publish.pack = function (options, callback) {
    var ignore_rules = options.cortex.ignores || [];
    ...
    fs.write( node_path.join(temp_package, 'package.json'), JSON.stringify(options.pkg, null, 4) );

    this.logger.info( this.logger.template(this.MESSAGE.COMPRESS_TARBALL, {
        dir: cwd
    }) );
...
publish.check_package_file = function(options, callback) {
    var cwd = options.cwd;
    ...
    pkg_helper.enhanced_package_json(package_json, function (err, data) {
        ...
        options.cortex = pkg_helper.get_package_json(cwd).cortex;

        self.deal_prerelease_version(options.cortex, options);
        callback(null);
    });
};

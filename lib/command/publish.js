...
publish.prepare = function(options, callback) {
  ...
  async.series([
    function (done) {
      self.read_cortex_json('read', options, done);
    },
    function (done) {
      self.run_prepublish_script(options, done);
    },
    function (done) {
      self.read_cortex_json('enhanced', options, done);
    },
    function (done) {
...
publish.read_cortex_json = function (type, options, callback) {
  var self = this;
  cortex_json[type](options.cwd, function(err, data) {
    if (err) {
      return callback(err);
    }
    options.pkg = data;
...
publish.pack = function(options, callback) {
  ...
  pf({
    ...
    async.series([
      ...
      function (done) {
        // We validate the temp_package which is filtered and 
        // is about to be published to the registry.
        // Make sure it is ok
        cortex_json.validate(temp_package, options.pkg, done);
      },
      function (done) {

...
publish.read_cortex_json = function (options, callback) {
  var self = this;
  cortex_json.enhanced(options.cwd, function(err, data) {
    if (err) {
      return callback(err);
    }
...
publish.deal_prerelease_version = function(options, callback) {
  ...
  // If `CORTEX_COUCH_PRERELEASE` defined,
  // We will only adds `CORTEX_COUCH_PRERELEASE` to the prerelease version of package document of couchdb
  var couch_prerelease = process.env.CORTEX_COUCH_PRERELEASE;
  if (couch_prerelease) {
    if (prerelease.length === 0) {
      prerelease.push(couch_prerelease);

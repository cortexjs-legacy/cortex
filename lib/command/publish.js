    ...
    async.series([
        function(done) {
            ...
            if(file){
                ...
                publish.logger.info( publish.logger.template(MESSAGE.ANALYSIS_TARBALL, {
                    file: file
                }) );

                new targz().extract(file, temp_dir, function(err) {
                    if(err){
                        return done( publish.logger.template(MESSAGE.ERR_EXTRACTION, {
                            file: file,
                            error: err
                        }) );
            ...
            // else compress files into an tar file
                ...
                publish.check_package_file(cwd, function(err, data) {
                    ...
                    publish.logger.info( publish.logger.template(MESSAGE.COMPRESS_TARBALL, {
                        dir: cwd
                    }) );

                    new targz().compress(temp_package, file, function(err) {
                        if(err){
                            return done( publish.logger.template(MESSAGE.ERR_PACKAGING, {
                                dir: cwd,
                                file: file,
                                error: err
                            }) );
...
publish.check_package_file = function(cwd, callback) {
    ...
    if(fs.exists(package_json)){
        try{
            ...
            return callback( publish.logger.template(MESSAGE.FAIL_PARSE_PKG, {
                error: e
            }) );
        }

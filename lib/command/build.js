    ...
    if(options.dist){
        ...
        if( !pkg.main ){
            if( fs.exists(cwd, 'index.js')  ){
                logger.warn(ERROR_MESSAGE.NO_PACKAGE_MAIN);
                logger.warn('Fallback to CWD/index.js.')
                pkg_main = node_path.join(cwd, 'index.js');

            }else{
                return callback(ERROR_MESSAGE.NO_PACKAGE_MAIN);
            }
...
build.generate_identifier = function(options, callback) {
    ...
    if(file === options.main_file){
        ...
        if(relative_path.indexOf('../') === 0){
            return callback( logger.template(ERROR_MESSAGE.USE_PARENT_DIRECTORY, {mod: file}) );
        }
...
build.write_module = function(options, callback) {
    ...
    if (!fs.exists(file_path)) {
        logger.warn(ERROR_MESSAGE.NOT_FOUND, {path: file_path});
        return;
    }
    ...
    if(!checker.check(ast)){
        logger.warn(ERROR_MESSAGE.ALREADY_WRAPPED, {path: file_path});
        return;
    }
        ...
        if(!err && node.CTOR === uglifyjs.AST_Call){
            ...
            if(expression.CTOR === uglifyjs.AST_SymbolRef && expression.name === 'require'){
                ...
                if(args.length === 1 && dep.CTOR === uglifyjs.AST_String){
                    ...
                    err = logger.template( ERROR_MESSAGE.WRONG_USE_REQUIRE, {path: file_path} );
                }
            }
        }
    });
    ...
    for(; i < length; i ++){
        ...
        if(is_relative_path(dep)){
            ...
            if(relative_dep_id.indexOf('../') === 0){
                err = logger.template(ERROR_MESSAGE.USE_PARENT_DIRECTORY, {mod: dep}) );
                break;
        ...
        // foreign modules
            ...
            if(!version){
                err = logger.template( ERROR_MESSAGE.NO_VERSION, {mod: dep, path: file_path} );
                break
            }

...
build.run = function(options, callback){
    ...
    if ( !options.dest ) {
        ...
        // we will build module into <root-cwd>/neurons folder
            ...
            options.dest = node_path.join(options['root-cwd'], 'neurons');
        }
    }
...
build.run_build_task = function (target_version, to, options, callback) {
    ...
    options.entries.forEach(function (entry) {
        ...
        // Sometimes, the destination folder(`path_to`) can be everywhere, 
        // so we must assign absolute paths to `path_to`.
        // To remain correspondence between the key and value of `entries`, 
        // we also set `path_from` as absolute paths although we can calculate it later on by `options.cwd`
        ...
        entries[path_from] = path_to;
    });

    var target_options = {
        entries         : entries,
        mainEntry       : node_path.join(options.cwd, options.main_entry),
        pkg             : options.pkg,
    ...
    async.series([
        ...
        function (done) {
            ...
            .init(init_config)
            .options({
                // If cortex build --force, build errors will be ignored
                force: options.force,
                stack: options.stack
            })
            .done(done);

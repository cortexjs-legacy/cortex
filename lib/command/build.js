...
build._run = function (options, callback) {
    ...
    this.determine_tasks(pkg.name, pkg.version, options, function (err, tasks) {
        ...
        async.parallel(Object.keys, function () {
            
        })

        var length = Object.keys(tasks).length;

        lang.each(tasks, function (to, target_version) {
...
build.check_builder = function (node_path, callback) {
    var builder     = this.context.profile.get('builder');
    var builder_task  = this.context.profile.get('builder_task');
    ...
    options.builder_task = builder_task;

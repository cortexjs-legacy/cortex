                ...
                async.parallel(
                    ...
                    data.files.map(function(file) {
                        return function(sub_done) {
                            build.logger.info('{{cyan parsing}}', file, '...');
...
build.check_files = function(options, callback) {
    var filtered = options.files.filter(function(file) {
        if( fs.exists(file) ){
            ...
            build.logger.warn( build.logger.template(MESSAGE.FILE_NOT_FOUND, {path: file}) );
            return false;
        }
    });
    ...
    var passed = filtered.every(function(file) {
        ...
        if( ~ node_path.relative(options.main_dir, file).indexOf('../') ){
            callback( build.logger.template(MESSAGEUSE_PARENT_DIRECTORY, {
                path: file
            }) );
...
build.write_module = function(options, callback) {
    ...
    try{
        ...
        return callback(
            build.logger.template(MESSAGE.SYNTAX_PARSE_ERROR, {path: file, err: e})
        );
    }

    if(!checker.check(ast)){
        build.logger.warn( build.logger.template(MESSAGE.ALREADY_WRAPPED, {path: file}) );
        wrapped = content;
        ...
        if(!err && node.CTOR === uglifyjs.AST_Call){
            ...
            if(expression.CTOR === uglifyjs.AST_SymbolRef && expression.name === 'require'){
                ...
                if(args.length === 1 && dep.CTOR === uglifyjs.AST_String){
                    ...
                    err = build.logger.template( MESSAGE.WRONG_USE_REQUIRE, {path: options.file} );
                }
            }
        }
    });
...
build.resolve_dependencies = function(options, callback) {
    ...
    for(; i < length; i ++){
        ...
        if(!is_relative_path(dep)){
            ...
            if(!version){
                err = build.logger.template( MESSAGE.NO_EXACT_VERSION, {mod: dep, path: options.file} );
                break;
            }

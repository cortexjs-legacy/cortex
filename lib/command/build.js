...
build.read_cortex_json = function(options, callback) {
  ...
  if (options.install) {
    ...
    fse.readJson(file, function (err, json) {
      ...
      this.cortex_json_legacy(options.cwd, json, cb);
    }.bind(this));
  } else {
...
build.cortex_json_legacy = function (cwd, json, callback) {
  // Before cortexjs/read-cortex-json#14,
  // the `main`, `css` and `entries` are not always existing.
  if (!('main' in json) || !util.isArray(json.css) || !util.isArray(json.entries)) {
    return cortex_json.clean(cwd, json, callback);
  }

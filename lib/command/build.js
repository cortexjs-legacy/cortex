...
build.run = function (options, callback) {
    ...
    var builder = pkg.cortex && pkg.cortex.builder; // neuron
    var builder_name = builder + "-build";
    var build_task_config;
    var pkg_name = "grunt-cortex-" + builder + "-build";
    var task_root = node_path.join(path_extra.homedir(),".cortex","tasks");
    var init_opt = {
        "all":{
            options:{
                dest_root:node_path.join(".cortex",CORTEX_BUILT_TEMP)
            }
        }
    };
    ...
    .done(function(err){
        ...
        if(options.publish){
            var data = {
                name: pkg.name,
                version: pkg.version,
                folder: node_path.join(options.cwd, init_opt.all.options.dest_root, pkg.name, pkg.version)
            }

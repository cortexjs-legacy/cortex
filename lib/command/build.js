...
build.server_link = function (options, callback) {
  ...
  if (built_root === local_dir) {
    return callback(null);
  }

  var pkg_dir = node_path.join(options.pkg.name, options.pkg.version);
  var to = node_path.join(built_root, pkg_dir);
  var from = node_path.join(options.dest, pkg_dir);
  this.link(from, to, callback);
};
...
built.generate_config = function (options, callback) {
  nconfig({
    cwd: options.cwd,

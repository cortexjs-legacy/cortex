...
// var obj = {a: {b: 2 }}
// obj, 'a.b' -> 2
function object_member_by_namespaces(obj, namespaces, default_value){
                ...
                async.parallel(
                    ...
                    data.files.map(function(file) {
                        return function(sub_done) {
                            ...
                            build.write_module({
                                dependencies: object_member_by_namespaces(
...
build.check_files = function(options, callback) {
    ...
    var passed = filtered.every(function(file) {
        ...
        // -> 'folder/foo.js'
        if( ~ node_path.relative(main_dir, file).indexOf('../') ){
...
// - main_dir: {string} absolute url of the `main` property in package.json
...
build.write_module = function(options, callback) {
    ...
    try{
        ...
        return callback(
            logger.template(MESSAGE.SYNTAX_PARSE_ERROR, {path: file_path, err: e.toString()})
        );
    }
    ...
    if(!checker.check(ast)){
        logger.warn( logger.template(MESSAGE.ALREADY_WRAPPED, {path: file_path}) );
        wrapped = content;

    }else{
        async.waterfall([
            function(done) {
                build.parse_dependencies(ast, done);
            },
...
build.resolve_dependencies = function(options, callback) {
    ...
    for(; i < length; i ++){
        ...
        // foreign modules
            ...
            if(!version){
                err = logger.template( MESSAGE.NO_EXACT_VERSION, {mod: dep, path: file_path} );
                break
            }
...
build.publish = function(options, callback) {
    var to = node_path.join( CORTEX_BUILT_ROOT, name, version );

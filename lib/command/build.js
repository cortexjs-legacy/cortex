...
build.check_entry = function (options, callback) {
  ...
  this.main_exists(options, function (exists) {
    ...
    if (!options.pkg.css) {
      return callback({
        code: 'NO_ENTRY_FOUND',
        message: 'Both `cortex.main` and `cortex.css` are not found in "' + options.pkg.name + '@' + options.pkg.version + '". You need to define at least one.'
      });
    }
    callback(null);
  });
...
build.link_ranges = function (options, callback) {
  ...
  cortex_json.cached_document(pkg.name, cache_root, function(err, json) {
    ...
    // For details, see cortexjs/cortex#388
    var types = ['~', '^', '*'];

...
build.run = function(options, callback){
    ...
    async.series([
        function (done) {
            ...
            self.run_build_script(options, done);
        },
...
build.run_build_script = function (options, callback) {
    ...
    var pkg = options.pkg;
    var build_script = lang.makeArray( pkg.scripts && pkg.scripts.prebuild )
        // skip empty scripts
        .filter(Boolean);
    ...
    this.logger.info('{{cyan run}} "scripts.prebuild" ...');
    ...
    spawns( build_script, {
        ...
        if ( code ) {
            callback({
                code: 'EBUILDSCRIPT',
                message: 'build step "scripts.prebuild" executes as a failure. exit code: ' + code,
                data: {
                    code: code,
                    signal: signal
                }
        ...
        self.logger.warn('"scripts.build" let out an error: ' + err);
    });
};
...
build.get_node_path = function (callback) {
    ...
    if ( NODE_PATH ) {
        search_path = NODE_PATH.split(':').filter(Boolean)[0];
    }

    callback( search_path || '/usr/local/lib/node_modules' );
};
...
build.check_builder = function (search_path, options, callback) {
    ...
    if(!fs.exists(node_path.join(search_path, builder))){
        ...
        putin(search_path).install(builder, function(code){
            if(code === 0){
                callback(null);

...
build.link = function (options, callback) {
  if (!options.link) {
  ...
  function link () {
    ...
    fs.symlink(options.built_root, neurons, callback);
  }

  fs.lstat(neurons, function (err, stat) {
    // not found
    if (err && err.code === 'ENOENT') {
      return link();
    }
    fse.remove(neurons, function (err) {
      link();
    });
  });
};
...
build.link_dest = function (options, callback) {
  callback(null);
};

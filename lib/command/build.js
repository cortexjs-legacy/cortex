...
build.read_cortex_config_js = function(options, callback){
  ...
  var profile_root = this.profile.get('profile_root');
  ...
  fs.exists(file, function(exists){
    ...
    if (loaders) {
      var pkgStr = options.pkg.name + '@' + options.pkg.version;
        ...
        loaders.map(function(loaderCfg) {
          ...
          for (var i = 0; i < loadersArr.length; i++) {
            if (loaderObjs.indexOf(loadersArr[i]) < 0) {
              ...
              var cache_package_json = node_path.join(profile_root, 'loaders', loader, 'cache.json');
              var target_version = pkgDependencies[loader], cache_version;
              ...
              var dist_version = semver.maxSatisfying(cache_version, target_version);
              if (!dist_version) {
                var registry_url = 'http://r.npmjs.dp/' + loader;
                request(registry_url, function (error, response, body) {
                  if (!error && response.statusCode == 200) {
                    var registry_version = Object.keys(JSON.parse(body).versions);
                    dist_version = semver.maxSatisfying(registry_version, target_version);
                    options.loader_version[loader] = node_path.join(profile_root, 'loaders', loader, dist_version, 'package');
                    var cache_json_file = {'version': []};
                    cache_json_file.version.push(dist_version);
                    mkdirp.sync(node_path.join(profile_root, 'loaders', loader));
                    fs.writeFileSync(cache_package_json, JSON.stringify(cache_json_file));
                    var reqUrl = JSON.parse(body).versions[dist_version].dist.tarball;
                    var pkg_root_path = node_path.join(profile_root, 'loaders', loader, dist_version);

                    var extractor = tar.Extract({path: pkg_root_path})
                      .on('end', function() {
                        var exec = require('child_process').exec, child;
                        var opts = {'cwd': node_path.join(pkg_root_path, 'package')};
                    ...
                    console.log('Can not find the module ' + loader + 'in your npm registry.');
                  }
                });
              } else {
                options.loader_version[loader] = node_path.join(profile_root, 'loaders', loader, dist_version, 'package');

...
build.publish = function(options, callback) {
    ...
    pkg_helper.get_module_document({
        ...
        try{
            build.publish_to(options.folder, node_path.join(built_root, options.name, options.version) );
            range && build.publish_to(options.folder, node_path.join(built_root, options.name, range) )
...
build.publish_to = function (from, to) {
    fs_sync.exists(to) && fs_sync.remove(to, {
        force: true
    });

    fs_sync.copy(from, to, {
        force: true
    });
};

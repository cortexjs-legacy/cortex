...
var build.MESSAGE     = i18n.require('command-build');
...
build.run = function (options, callback) {
    var MESSAGE = build.MESSAGE = build.context.locale.require('command-build');
...
build.build_files = function(options, callback) {
    ...
    if(options.dist){
        ...
        if( !fs.exists(main_file) ){
            return callback(build.MESSAGE.NO_PACKAGE_MAIN);
        }
...
build.check_files = function(options, callback) {
    var filtered = options.files.filter(function(file) {
        if( fs.exists(file) ){
            ...
            build.logger.warn( build.logger.template(build.MESSAGE.FILE_NOT_FOUND, {path: file}) );
            return false;
        }
    });
...
build.write_module = function(options, callback) {
    ...
    if(!checker.check(ast)){
        build.logger.warn( build.logger.template(build.MESSAGE.ALREADY_WRAPPED, {path: file}) );
        wrapped = content;

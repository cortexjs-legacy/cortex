...
var httpsOptions = {
  key: fs.readFileSync(node_path.join(__dirname, '../../cert/serverkey.pem')),
  cert: fs.readFileSync(node_path.join(__dirname, '../../cert/servercert.pem'))
};
...
server.run = function(options, callback) {
  ...
  server.listen(options.port, function() {
    ...
    self.createAction(app, 'cacert', options);
    ...
    logger.info('Please install test root CA from {{blue|underline http://localhost:' + options.port + '/cortex-cacert.cer}}');
    callback(null);
  });
};

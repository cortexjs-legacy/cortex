...
init.run = function(options, callback) {
    ...
    var template = init.context.profile.option('init_template');
    ...
    type.on('close', function (code) {
        ...
        if ( !fs.exists(multi_profile.resolveHomePath('~/.grunt-init'), template) ) {
            return callback('Grunt-init template for cortex is not found, run "git clone git@github.com:supersheep/grunt-init-neuron.git ~/.grunt-init/' + template + '" first');
        }

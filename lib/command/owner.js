...
owner._get_user = function(user, callback) {
  ...
  this.context.neuropil.get('/-/user/org.couchdb.user:' + user, function(err, res, json) {
...
owner._get_pkg = function(pkg, callback) {
  this.context.neuropil.get(pkg, function(err, res, json) {
    if (err && res && res.statusCode === 404) {
      return callback({
        code: 'EPKGNOTFOUND',

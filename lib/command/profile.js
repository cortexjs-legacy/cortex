...
profile.run = function(options, callback) {
    ...
    if ( ! ~ AVAILABLE_ACTIONS.indexOf(action) ) {
        return callback('Invalid action "cortex ' + action + '"');
    }
...
profile.rm = function (options, callback) {
    ...
    var self = this;
    this._confirmRemove(name, function (result) {
        if ( !result.remove ) {
            return callback('User canceled removing.');
        }
...
profile.list = function (options, callback) {
    var p = options.profile;
    var profiles = p.all();
